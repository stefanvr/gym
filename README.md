...
See [doc/spec/domain/gym.md](doc/spec/domain/gym.md) for what it does and the rules it follows, and
[doc/spec/tech/architecture.md](doc/spec/tech/architecture.md) for technical decisions. This
project develops under the [Harness](.harness/README.md) — see `.harness/workflow/WORKFLOW.md` for
...
[doc/spec/domain/gym.md](doc/spec/domain/gym.md) for the full field reference. Everything else is
generated or static; [doc/spec/tech/architecture.md](doc/spec/tech/architecture.md) has the details.
...
npm run test:e2e      # Playwright smoke tests, desktop + mobile (needs Node >=20, see doc/setup-dev-env.md)
...
None currently. See the [Software Design guide](.harness/guides/software-design.md)'s "Development
Affordances" section for how one gets added and documented here if that changes.
...
coordinates should be unique) live in [doc/spec/domain/gym.md](doc/spec/domain/gym.md) — this is just
...
Design tokens are in [doc/spec/style/foundation.md](doc/spec/style/foundation.md) — but note
discipline colors are also hard-coded in `js/map.js` (inline-SVG markers can't read CSS variables);
see [doc/spec/tech/architecture.md](doc/spec/tech/architecture.md)'s "Discipline colors live in two
files" decision before
