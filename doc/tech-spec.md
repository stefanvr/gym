...
**Why:** domain-spec.md §3 documents two real incidents where a copy-pasted file kept the
source's coordinates unnoticed until someone diffed the JSON by hand.

**Rejected:** warning instead of failing. A duplicate coordinate is never intentional (see
domain-spec.md §1 — "a gym's identity is its coordinates, in practice") and always hides a real
venue, so there's no legitimate case to only warn about.
